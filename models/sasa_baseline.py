...
class RelativeEmbeddings2d(nn.Module):
    def __init__(self, extent, embedding_size):
        ...
        self.width_mat = nn.Parameter(torch.randn((1, embedding_size // 2, 1, extent, 1, 1)), requires_grad=True)
        self.height_mat = nn.Parameter(torch.randn((1, embedding_size // 2, extent, 1, 1, 1)), requires_grad=True)
    ...
    def forward(self, x):
        ...
        x_h, x_w = x.view(N, self.embedding_size, self.extent, self.extent, H, W) \
            .split(self.embedding_size // 2, dim=1)
        y_h = x_h + self.height_mat.expand_as(x_h)
        y_w = x_w + self.width_mat.expand_as(x_w)
        return torch.cat((y_h, y_w), dim=1).view(N, CKK, HW)
...
class SASAConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        # TODO
        #  ------------------------------------------------------------------------------------------------
        #  EASIEST TO READ BUT SLOWEST
        #  q = q.view(N, self.heads, self.out_channels // self.heads, 1, HW)
        #  weights = F.softmax(einsum('ngcuf,ngcuf->nguf', q.expand_as(k), k), dim=-2)
        #  attn_maps = einsum('ngcuf,ngcuf->ngcf', weights.unsqueeze(2).expand_as(v), v)
        #  ------------------------------------------------------------------------------------------------
        #  ALSO READABLE BUT SLOWER
        #  q = einsum('ngcf->ngfc', q)
        #  k = einsum('ngcuf->ngfcu', k)
        #  v = einsum('ngcuf->ngfuc', v)
        #  weights = F.softmax(q.unsqueeze(-2).matmul(k).squeeze(-2), dim=-1)  # ngfu
        #  attn_maps = einsum('ngfc->ngcf', x.unsqueeze(-2).matmul(y).squeeze(-2))
        #  ------------------------------------------------------------------------------------------------
...
class SASABaseline(nn.Module):
    def __init__(self, stem=True):
        ...
        self.l2 = nn.Sequential(
            SASAConv2d(in_channels=256, out_channels=512, kernel_size=7, heads=16, stride=1),
            nn.LeakyReLU(),
            nn.AvgPool2d(kernel_size=2, stride=2),
            nn.BatchNorm2d(512),
        )
